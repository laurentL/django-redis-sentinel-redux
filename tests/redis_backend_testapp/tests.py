...
class DjangoRedisConnectionStrings(TestCase):
    def setUp(self):
        ...
        self.cf = pool.get_connection_factory(options={'SENTINEL_SERVICE_NAME': 'rmaster'})
...
class DjangoRedisCacheTestCustomKeyFunction(TestCase):
    def setUp(self):
        ...
        settings.CACHES['default']['REVERSE_KEY_FUNCTION'] = (
            'redis_backend_testapp.tests.reverse_key'
        )
...
class DjangoRedisCacheTests(TestCase):
    ...
    def test_save_dict(self):
        if isinstance(self.cache.client._serializer, json_serializer.JSONSerializer):
            self.skipTest('Datetimes are not JSON serializable')

        if isinstance(self.cache.client._serializer, msgpack_serializer.MSGPackSerializer):
    ...
    def test_ttl(self):
        if FAKE_REDIS:
            raise unittest.SkipTest(
                'FakeRedis ttl is broken, see https://github.com/jamesls/fakeredis/issues/119',
            )
    ...
    def test_master_slave_switching(self):
        if isinstance(self.cache.client, SentinelClient):
