...
class SentinelClient(DefaultClient):
    """
    Modifies DefaultClient to work on Sentinel Cluster. URLs passed as servers are no longer master
    on index 0 and slaves the following ones. All URLs should represent the list of sentinels,
    where order no matters anymore. It does not use any cached ConnectionPool as
    SentinelConnectionPool is for sentinels, not master and slaves. The Sentinel client creates a
    StrictRedis client that performs the connections to actual current elected master or
    slave instances, instead of indexing the URLs for using them as a fixed way to connect
    to each server. This way, through Sentinel client instead of direct creation of
    StrictRedis from URLs, we always have a valid master or slave client (before or after failover).
    New OPTIONS:
        - SENTINEL_SERVICE_NAME (required): Name of monitored cluster
        - SENTINEL_SOCKET_TIMEOUT (optional): Socket timeout for connecting to sentinels,
        in seconds (accepts float)
    """
    ...
    def get_client(self, write=True, force_slave=False, **kwargs):
        """
        ...
        If read always looks for a slave (round-robin algorithm, with fallback to master
        if none available)
    ...
    def connect(self, master=True, force_slave=False):
        ...
        instance. Sentinel always give a valid StrictRedis client with fallback to master in case
        of no slaves.
        No caching done with clients.
        Even though it can be an improvement, it could lead to stale invalid clients in failovers.
        Maybe in the future.
        """
        if master:
